<template>
    <section class="section-panel">
        <div class="section-header">
            <div class="header"><i class="el-icon-user"></i> Approver</div>
            <small class="tw-italic">You can add one approver only</small>
        </div>
        <div class="section-body">
            <div class="tw-mb-8">
                <div class="tw-flex tw-items-center tw-justify-between tw-px-4 tw-rounded tw-bg-gray-50 tw-shadow tw-py-2" 
                    v-for="(approver, key) in invitations" :key="key">
                    <div class="tw-flex tw-items-center">
                        <el-avatar :src="approver.avatar">
                        </el-avatar>
                        <div class="tw-mx-4">{{approver.email}}</div>
                        <el-tooltip effect="dark" content="Pending" v-if="approver.status == 'Pending'">
                    ...
                    <div class="tw-flex tw-items-center" v-if="$route.params.role != 'request' && currentRole != 'Collaborator'">
                        <el-popover placement="top" width="auto" trigger="click" v-if="approver.link">
                            ...
                            <el-tag size="medium" class="tw-cursor-pointer tw-mr-4" slot="reference"
                                @click="clipboardLink(approver.link)">
                                Copy Link</el-tag>
                        </el-popover>
                    </div>
                </div>
                <div class="tw-flex tw-items-center tw-justify-center tw-px-4 tw-rounded tw-bg-gray-50 tw-shadow tw-py-4" v-if="invitations.length == 0">
                    <div>There is no Approver added yet</div>
                </div>
            </div>
            <el-row :gutter="20" v-if="$route.params.role != 'request' && currentRole != 'Collaborator'">
                    ...
                    <el-form :model="formData" class="tw-flex" @submit.native.prevent="submitForm('dialogForm')"
                        ...
                        <el-form-item prop="approver">
                            <el-autocomplete id="email" popper-class="client-autocomplete" style="width:100%"
                                v-model="formData.approver" :fetch-suggestions="fetchClients" :trigger-on-focus="false"
                                placeholder="Invite Approver (add new email to update approver)" @select="handleSelect">
                                <template slot-scope="{ item }">
                                    <div class="autocomplete-item">
                                        <div class="autocomplete-content">
                                            <div class="project-name"
                                                v-html="$options.filters.highlight(item.name, formData.approver)">
                                                {{ item.name }}</div>
                                            <div class="footer-content">
                                                <div class="project-company"
                                                    v-html="$options.filters.highlight(item.email, formData.approver)">
                                                    {{ item.email }}</div>
                        ...
                        <el-button class="tw-ml-4" icon="el-icon-position" :loading="saveLoading" @click="submitForm('dialogForm')" type="primary">Send</el-button>
                    </el-form>
                </el-col>
...
<script>
    ...
    export default {
        ...
        ...mapGetters(['currentRole']),
            invitations() {
                let inv = this.project.project_invitations.filter(inv => (inv.user_id || inv.email) && inv.role == 'client').map(inv => {
                    return {
                        id: inv.id,
                        user_id: inv.user_id,
                        email: inv.email,
        ...
        methods: {
            ...
            handleSelect(item) {
                this.formData.approver = item.email;
            },
